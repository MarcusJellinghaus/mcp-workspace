...
5. **`github_search` stops before the surplus item (step 5).** The `i >= max_results`
   ...
   only to discard it. `islice(results, max(0, max_results))` stops without that pull, so
   `totalCount` really does cost nothing extra. The `max(0, ...)` preserves the old
   behaviour for a negative `max_results`, which the `enumerate` guard silently treated as
   "collect nothing" but `islice` rejects with `ValueError`.
