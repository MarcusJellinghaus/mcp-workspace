...
| Debug logging | Single debug line with command args + signing-related config (`commit.gpgsign`, `gpg.format`, `user.signingkey`) read via `repo.config_reader().get_value(..., default="<unset>")`. On `GitCommandError`, log truncated stderr + exit status. Safety net since the test is mock-only. |
...
2. **`step_2.md`** — Add `error_category` field to `CommitResult` (typed `Literal`), thread `error_category=None` through all 7 existing dict-literal returns in `commits.py` and `workflows.py`. Mechanical plumbing; no behavior change. Mypy `total=True` enforces completeness.
