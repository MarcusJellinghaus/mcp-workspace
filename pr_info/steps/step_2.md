...
> section, replacing the now-unused `remaining` local with `total`. Keep the literal em
> dash character already used in that file. Do not extract a shared helper with
> `github_operations/formatters.py` — the summary explains why.
>
