# Step 3 — `glob_note` for brace globs that match no files

Read [summary.md](./summary.md) first.

Braces are the one member of the silent-zero family with no structural signal: gitwildmatch
compiles `{a,b}` to a valid regex matching the braces literally, with `include is True`.
Detection is therefore textual, and it cannot raise — `{{cookiecutter.project_slug}}` is a
real directory name and no escape mechanism exists. Every other member, unterminated
bracket expressions included, raises in `_match_glob` (step 2).

## WHERE

- `tests/file_tools/test_search.py` — new class `TestSearchFilesGlobNote`
- `src/mcp_workspace/file_tools/search.py` — new module-level constant `_BRACE_NOTE` plus a
  `_glob_note(glob) -> Optional[str]` helper; `search_files` computes `glob_note` once and
  attaches it to both return paths
...
Assert on `"brace expansion" in result["glob_note"].lower()` for the brace cases, and
`"glob_note" not in result` for absence.

## HOW

The constant sits beside the existing `_MAX_LINE_CHARS` constant. Its text names the cause
...
`_glob_note` returns `_BRACE_NOTE` when `"{" in glob`, else `None`. Brackets are not its
concern: an unterminated `[` already raised in `_match_glob`, and a closed class such as
`[!a]*.py` is a working pattern whose zero matches are genuine.
...
`feat(search): flag brace globs that match no files`
...
> fails. Then add `_BRACE_NOTE`, the `_glob_note` helper, and the
...
> still pass. Commit as `feat(search): flag brace globs that match no files`.
